...
const schema = z.object({
  ...
  principal: z.string().refine((value) => Number(value) > 0, "Principal must be greater than 0"),
...
export const FixedDepositForm = ({ deposit, onSaved, onCancel }: Props) => {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      fd_id: deposit?.fd_id ?? "",
      bank: deposit?.bank ?? "",
      principal: deposit ? String(deposit.principal) : "",
      ...
      <form onSubmit={form.handleSubmit(submit)} className="space-y-5">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <FormField control={form.control} name="principal" render={({ field }) => (
            <FormItem><FormLabel>Principal (₹)</FormLabel><FormControl><Input type="number" min="0" step="0.01" placeholder="200000" {...field} /></FormControl><FormMessage /></FormItem>
          )} />
        ...
        <div className="grid grid-cols-1 gap-3 border-y py-4 sm:grid-cols-2">
          <div><p className="text-xs text-muted-foreground">Tenure</p><p className="text-lg font-semibold tabular-nums">{preview.tenure.toFixed(2)} months</p></div>
